...
class TNet(Layer):
    def __init__(self, add_regularization=False, bn_momentum=0.90, **kwargs):
        super(TNet, self).__init__(**kwargs)
        self.add_regularization = add_regularization
        self.bn_momentum = bn_momentum
        self.conv0 = CustomConv(64, (1, 1), strides=(1, 1), apply_bn=False, bn_momentum=bn_momentum)
        self.conv1 = CustomConv(128, (1, 1), strides=(1, 1), apply_bn=False, bn_momentum=bn_momentum)
        self.conv2 = CustomConv(1024, (1, 1), strides=(1, 1), apply_bn=False, bn_momentum=bn_momentum)
...
class CustomConv(Layer):
    def __init__(self, filters, kernel_size, strides, padding='valid', activation=None,
                 apply_bn=False, bn_momentum=0.90, **kwargs):
...
class CustomDense(Layer):
    def __init__(self, units, activation=None, apply_bn=False, bn_momentum=0.90, **kwargs):
...
def get_ae_model(input_shape, bn_momentum=0.99):
    end_points = {}
    ...
    net = CustomDense(512, activation=tf.nn.relu, apply_bn=True,
                             bn_momentum=bn_momentum)(global_descriptor)

    net = CustomDense(512, activation=tf.nn.relu, apply_bn=True,
                             bn_momentum=bn_momentum)(net)
    net = CustomDense(input_shape*3, activation=None, apply_bn=True,
                             bn_momentum=bn_momentum)(net)

    net = tf.reshape(net, [tf.shape(net)[0], -1, 3])
    return Model(inputs=pt_cloud, outputs=net)

def make_encoder_model(input_shape, z_dim):
    pt_cloud =tf.keras.Input(shape=(input_shape, 3), dtype=tf.float32)
    pt_cloud_transform = TNet(add_regularization=True)(pt_cloud)
    pt_cloud_transform = tf.expand_dims(pt_cloud_transform, axis=2)   
    hidden_64 = CustomConv(64, (1, 1), strides=(1, 1), activation=tf.nn.relu, apply_bn=False)(pt_cloud_transform)
    embed_64 = CustomConv(128, (1, 1), strides=(1, 1), activation=tf.nn.relu, apply_bn=False)(hidden_64)
    embed_64 = tf.squeeze(embed_64, axis=2)
    embed_64_transform = TNet(add_regularization=True)(embed_64)
    embed_64_transform = tf.expand_dims(embed_64_transform, axis=2)
    hidden_64 = CustomConv(256, (1, 1), strides=(1, 1), activation=tf.nn.relu, apply_bn=False)(embed_64_transform)
    hidden_128 = CustomConv(256, (1, 1), strides=(1, 1), activation=tf.nn.relu, apply_bn=False)(hidden_64)
    embed_1024 = CustomConv(512, (1, 1), strides=(1, 1), activation=tf.nn.relu, apply_bn=False)(hidden_128)
    embed_1024 = tf.squeeze(embed_1024, axis=2)
    global_descriptor = tf.reduce_max(embed_1024, axis=1)    
    mean = CustomDense(z_dim,activation=None, apply_bn=False)(global_descriptor)
    var = CustomDense(z_dim, activation=None, apply_bn=False)(global_descriptor)
    return tf.keras.Model(inputs=pt_cloud, outputs=[mean, var])


def make_decoder_model(input_shape, z_dim):
    inputs = tf.keras.Input(shape=(z_dim,))
    net = CustomDense(z_dim, activation=tf.nn.relu, apply_bn=False)(inputs)
    net = CustomDense(512, activation=tf.nn.relu, apply_bn=False)(net)
    net = CustomDense(1024, activation=tf.nn.relu, apply_bn=False)(net)
    net = CustomDense(input_shape*3, activation=None, apply_bn=False)(net)
    net = tf.reshape(net, [tf.shape(net)[0], -1, 3])
    return tf.keras.Model(inputs=inputs, outputs=net)

def make_discriminator_model(z_dim):
    inputs = tf.keras.Input(shape=(z_dim,))
...
def make_binary_encoder_model():
    ...
    embed_1024 = tf.squeeze(embed_1024, axis=2)
    global_descriptor = tf.reduce_max(embed_1024, axis=1)    
    mean = CustomDense(Z_DIM, activation=tf.keras.activations.sigmoid, apply_bn=False)(global_descriptor)
    return tf.keras.Model(inputs=pt_cloud, outputs=mean)
